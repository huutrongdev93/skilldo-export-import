...
class AdminExportAjax {
    static function products($ci, $model): void
    {
        $result['status'] 	= 'error';
...
Ajax::admin('AdminExportAjax::products');


class AdminImportAjax {
    static function productsUpload($ci, $model): void
    {
        $result['status'] 	= 'error';
...
//                }
                ...
                foreach ($schedules as $numberRow => $schedule) {
                    ...
                    $rowData = AdminImportProduct::creatData($schedule);
    ...
    static function productsImport($ci, $model): void
    {
        $result['status'] 	= 'error';
        ...
        if(Request::post()) {
            ...
            foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($dataPath.'upload')) as $file) {
                ...
                if(Str::is('0\\*', $fullPath)) {
                    foreach ($products as $product) {
                        ...
                        if($checkVariation != 0) {
                            $failed[] = [
                                'id'        => $product->id,
                                'title'     => $product->title,
                                'message'   => 'Sản phẩm này có nhóm sản phẩm',
                            ];
                            continue;
                        }
                        $categoriesUpdate = [];
                    ...
                    if (empty($defaultId)) {
                        $failed[] = [
                            'id' => $products[1]->parent_id,
                            'title' => $products[1]->title,
                            'message' => 'Không xác định được sản phẩm chính của nhóm sản phẩm ' . $products[1]->parent_id,
                        ];
                        continue;
                    }
                    ...
                    if (!empty($attributesItemsNew) || !empty($attributesNew)) {
                        ...
                        if (!empty($attributesNew)) {
                            foreach ($attributesNew as $attrName => $attrItemList) {
                                $attr = Attributes::get(Qr::set('title', $attrName)->select('id', 'title'));
                                if (!have_posts($attr)) {
                                    foreach ($products as $product) {
                                        $failed[] = [
                                            'id' => $product->id,
                                ...
                                $attrItems = Attributes::getsItem(Qr::set('option_id', $attr->id)->whereIn('title', $attrItemList)->select('id', 'option_id', 'title'));
                        ...
                        if (!empty($attributesItemsNew)) {
                            ...
                            foreach ($attributesItemsNew as $attrId => $attrItemList) {
                                $attrItems = Attributes::getsItem(Qr::set('option_id', $attrId)->whereIn('title', $attrItemList));
                                if (!have_posts($attrItems)) {
                                    foreach ($products as $product) {
                                        $failed[] = [
                                            'id' => $product->id,
                ...
                if(Str::is('0\\*', $fullPath)) {
                    ...
                    if(is_numeric($products[1]->parent_id)) {
                        ...
                        foreach ($attributesNew as $attrId => $attrData) {
                            ...
                            $attrItems = Attributes::getsItem(Qr::set('option_id', $attrId)->whereIn('title', $attrData['items'])->select('id', 'option_id', 'title'));
                            if(!have_posts($attrItems)) {
                                foreach ($products as $product) {
                                    $failed[] = [
                                        'id' => $product->id,
                            ...
                            if(count($attrData['items']) != count($attrItems)) {
                                foreach ($products as $product) {
                                    $failed[] = [
                                        'id' => $product->id,
                                        'title' => $product->title,
                        ...
                        $categoriesUpdate = [];
                        if(!empty($productDefault['categories'])) {
                            $categories = ProductCategory::gets(Qr::set()->whereIn('name', $productDefault['categories'])->select('id', 'name'));
                            if(!have_posts($categories)) {
                                $failed[] = [
                                    'id'        => $product->id,
                                    'title'     => $product->title,
                            ...
                            $checkError = true;
                            foreach ($productDefault['categories'] as $categoryName) {
                                $checkError = true;
                                foreach ($categories as $cate) {
                                    if($cate->name == $categoryName) {
                                        $categoriesUpdate[] = $cate->id;
...
Ajax::admin('AdminImportAjax::productsUpload');
Ajax::admin('AdminImportAjax::productsImport');